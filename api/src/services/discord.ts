...
export class DiscordService {
  ...
  static async getUserGuilds(accessToken: string, userId: string): Promise<UserGuild[]> {
    ...
    // Get bot's guilds from Redis Set for O(1) lookups
    const botGuildIds = await redis.smembers(REDIS_KEYS.GUILD_SET);
    ...
    const accessibleGuilds = userGuilds
      .filter(userGuild => botGuildIdSet.has(userGuild.id))
      .map(guild => ({
        id: guild.id,
        name: guild.name,
        icon: guild.icon,
        ...
        botHasAccess: true
        // features field is intentionally removed
      }));

    // Cache the filtered guilds for 5 minutes
    await redis.setex(cacheKey, CACHE_TTL.USER_GUILDS, JSON.stringify(accessibleGuilds));
  ...
  static async getGuildLastUpdate(guildId: string): Promise<number | null> {
    const lastUpdated = await redis.hget(REDIS_KEYS.GUILD_INFO(guildId), 'lastUpdated');
