...
export class ShardAssignmentService {
  ...
  calculateShardDistribution(
    ...
    logger.info(
      `[ShardAssignmentService] Calculating distribution for ${totalShards} shards across ${totalClusters} clusters (${effectiveShardsPerCluster} shards per cluster)`
    );
    ...
    for (let clusterId = 0; clusterId < totalClusters; clusterId++) {
      ...
      const endShard = Math.min(
        startShard + effectiveShardsPerCluster - 1,
        totalShards - 1
      );
      ...
      logger.debug(
        `[ShardAssignmentService] Cluster ${clusterId}: shards [${shards.join(
          ", "
        )}]`
      );
    }
    ...
    for (
      let shardId = totalClusters * effectiveShardsPerCluster;
      shardId < totalShards;
      shardId++
    ) {
    ...
    if (remainingShards.length > 0) {
      logger.warn(
        `[ShardAssignmentService] Found ${
          remainingShards.length
        } remaining shards: [${remainingShards.join(", ")}]`
      );
    }
  ...
  assignShardsToCluster(
    ...
    const distribution = this.calculateShardDistribution(
      totalShards,
      shardsPerCluster
    );
  ...
  getShardAssignment(
    ...
    const shards = this.assignShardsToCluster(
      clusterId,
      totalShards,
      shardsPerCluster
    );
    ...
    const endIndex = Math.min(
      startIndex + effectiveShardsPerCluster - 1,
      totalShards - 1
    );
  ...
  getAllShardAssignments(
    ...
    for (
      let clusterId = 0;
      clusterId < distribution.totalClusters;
      clusterId++
    ) {
      const assignment = this.getShardAssignment(
        clusterId,
        totalShards,
        shardsPerCluster
      );
  ...
  validateShardDistribution(distribution: ShardDistribution): boolean {
    ...
    for (const shards of clusterShards.values()) {
      for (const shardId of shards) {
        if (assignedShards.has(shardId)) {
          logger.error(
            `[ShardAssignmentService] Duplicate shard assignment: ${shardId}`
          );
          return false;
        }
    ...
    for (const shardId of remainingShards) {
      if (assignedShards.has(shardId)) {
        logger.error(
          `[ShardAssignmentService] Duplicate remaining shard: ${shardId}`
        );
        return false;
      }
      assignedShards.add(shardId);
    ...
    for (let shardId = 0; shardId < totalShards; shardId++) {
      if (!assignedShards.has(shardId)) {
        logger.error(
          `[ShardAssignmentService] Missing shard assignment: ${shardId}`
        );
        return false;
      }
    }
    ...
    for (const shardId of assignedShards) {
      if (shardId >= totalShards) {
        logger.error(
          `[ShardAssignmentService] Invalid shard assignment: ${shardId} (total: ${totalShards})`
        );
        return false;
      }
    }
    ...
    logger.info(
      `[ShardAssignmentService] Shard distribution validation passed`
    );
    return true;
  }
   ...
   */
  rebalanceShards(
    oldDistribution: ShardDistribution,
    newTotalShards: number
  ): ShardDistribution {
    logger.info(
      `[ShardAssignmentService] Rebalancing shards from ${oldDistribution.totalShards} to ${newTotalShards}`
    );

    const newDistribution = this.calculateShardDistribution(
      newTotalShards,
      oldDistribution.shardsPerCluster
    );
    ...
    for (
      let clusterId = 0;
      clusterId <
      Math.max(oldDistribution.totalClusters, newDistribution.totalClusters);
      clusterId++
    ) {
      ...
      if (
        oldShards.length !== newShards.length ||
        !this.arraysEqual(oldShards, newShards)
      ) {
        logger.info(
          `[ShardAssignmentService] Cluster ${clusterId}: [${oldShards.join(
            ", "
          )}] -> [${newShards.join(", ")}]`
        );
      }
  ...
  getOptimalShardsPerCluster(
    ...
    const configuredShardsPerCluster = parseInt(
      process.env.SHARDS_PER_CLUSTER || "16"
    );
    const optimalShardsPerCluster = Math.min(
      configuredShardsPerCluster,
      maxConcurrency
    );
    ...
    if (maxClusters > 10) {
      ...
      logger.info(
        `[ShardAssignmentService] Adjusted shards per cluster to ${adjustedShardsPerCluster} to limit cluster count (Discord max concurrency: ${maxConcurrency})`
      );
      return adjustedShardsPerCluster;
    }

    logger.info(
      `[ShardAssignmentService] Using ${optimalShardsPerCluster} shards per cluster (Discord max concurrency: ${maxConcurrency})`
    );
