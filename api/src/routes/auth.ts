...
router.get('/discord/callback', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Get user guilds
    const userGuilds = await DiscordService.getUserGuilds(tokenData.access_token, userInfo.id);

    console.log('User guilds from Discord:', userGuilds);

    // Store user session
    const sessionId = SessionService.generateSessionId();
    await SessionService.storeUserSession(sessionId, {
      user: userInfo as User,
      guilds: userGuilds as UserGuild[],
...
router.get('/user/guilds', (req: Request, res: Response) => {
  ...
  console.log('User guilds from session:', authReq.user.guilds);
